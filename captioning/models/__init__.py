...
from .XDualRelation import DualRelationModelX2
...
def setup(opt):
    ...
    # Simple attention model + ON-LSTM
    elif opt.caption_model == 'onatt':
        model = ONModel(opt)
    # Top-down attention model + ON-LSTM
    elif opt.caption_model == 'updownon':
        model = UpDownONModel(opt)
    # MultiModal ON-LSTM
    elif opt.caption_model == 'm2on':
        model = M2ONModel(opt)
    # StackAtt
    ...
    # my model
    elif opt.caption_model == 'dualearly':
        model = DualRelationModelEarlyFusionModel(opt)
    elif opt.caption_model == 'dual_s':
        model = DualRelationModelModel(opt)
    elif opt.caption_model == 'dual_late_att':
        model = DualRelationModelLateAttention(opt)
    elif opt.caption_model == 'dual_s_gcn':
        model = DualRelationModelGCNLike(opt)
    
    # my X model
    elif opt.caption_model == "x2":
        model = DualRelationModelX2(opt)
