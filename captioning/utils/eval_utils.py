...
from tree_distance.paraTED import ParaTED
...
def language_eval(dataset, preds, preds_n, eval_kwargs, split):
    ...
    eval_tree = eval_kwargs.get('eval_tree', 0)
    ...
    # === eval tree structure by sentence transformer + tree edit distance ====
    if eval_tree:
        ...
        for i, p in enumerate(preds_filt):
            image_id, para_pred = p['image_id'], p['caption']
            para_gt = coco.imgToAnns[image_id][0]['caption']
            ted_score = pted.compute_score(para_gt, para_pred)
            all_ted.append(ted_score)
            imgToEval[image_id]['ted'] = ted_score
            if i % 200 == 0:
                print('ted progress: {}/{}'.format(i, len(preds_filt)))
        out['ted'] = sum(all_ted) / len(all_ted)
